...
public class BusActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bnum=(EditText) findViewById(R.id.editText2);
        snum=(EditText) findViewById(R.id.editText4);
        bsnum=(EditText)findViewById(R.id.editText5);
    ...
    public void mycontinue(View view){
        if(bnum.length()==0){
            bnum.setError("Enter Bus Number");
        }
        else if(snum.length()==0){
            snum.setError("Enter Seat Number");
        }
        else if(bsnum.length()==0){
            bsnum.setError("Enter PNR Number");
        }
        else {
            bnum=(EditText) findViewById(R.id.editText2);
            snum=(EditText) findViewById(R.id.editText4);
            bsnum=(EditText)findViewById(R.id.editText5);
            int busno = Integer.parseInt(bnum.getText().toString());
            int seatno=Integer.parseInt(snum.getText().toString());
            String bsno=bsnum.getText().toString();
            contentValues.put("BUS_NO",busno);
            contentValues.put("Seat_No",seatno);
            if(bsno.length()!=0){
                contentValues.put("BUS_STOP_NO ",bsno);
            }
            ...
            try{
                ...
                long result= db.insert("BusWaterOrder",null,contentValues);
                Cursor cursor= db.query("BusWaterOrder",new String[]{"Id","Category"},null,
                        null,null,null,null);
                if(cursor.moveToLast()){
                    category=cursor.getString(1);
                }
                if(result==-1){
                    Toast toast2= Toast.makeText(BusActivity.this,
                            "Data is not loaded successfully inside the BusWaterTable", Toast.LENGTH_SHORT);
                    toast2.show();
                }else {
                    Toast toast3= Toast.makeText(BusActivity.this,
                            "Database is  loaded successfully inside the BusWaterTable ", Toast.LENGTH_SHORT);
                    toast3.show();}
                cursor.close();
                db.close();
            } catch (SQLiteException e){
                Toast toast1= Toast.makeText(BusActivity.this,
                        "Database is not access successfully", Toast.LENGTH_SHORT);
                toast1.show();
            }
            Toast toast= Toast.makeText(BusActivity.this,
                    "Record is Loaded successfully", Toast.LENGTH_SHORT);
            toast.show();
            if(category.equals("Water")){
                Intent intent1=new Intent(this,WaterOrderActivity.class);
                intent1.putExtra(WaterOrderActivity.BUS_WATER,BUSWATER);
                startActivity(intent1);
            } else {
                Intent intent=new Intent(this,MixOrderActivity.class);
                intent.putExtra(MixOrderActivity.BUS_MIX,BUSMIX);
