...
public class WaterOrderActivity extends AppCompatActivity {
    ...
    public static final String PLATFORM_WATER="PLATFORMWATER";
    public Intent intent30;
    public int BUSWATER;
    public int TRAINWATER;
    public int PLATFORMWATER;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent30.hasExtra(PLATFORM_WATER)){
            PLATFORMWATER=intent30.getIntExtra(PLATFORM_WATER,0);
        }
    }
    ...
    public void mypayment(View view) {
        ...
        // here is the code form the platform passengers
        ...
        if (PLATFORMWATER == 3) {
            ...
            try {
                ...
                long result = db.update("PlatformWaterOrder", content1, "Id= ?",
                        new String[]{Integer.toString(id)});
                if (result == -1) {
                    Toast toast = Toast.makeText(this, "Data is not uploaded in side the PlatformWaterOrder Table", Toast.LENGTH_SHORT);
                    toast.show();
                } else {
                    Toast toast = Toast.makeText(this, "Data is  uploaded in side the PlatformWaterOrder Table", Toast.LENGTH_SHORT);
                    toast.show();
                }
                db.close();
            } catch (Exception e) {
            ...
            Intent intent = new Intent(this, PaymentModeActivity.class);
            intent.putExtra(PaymentModeActivity.TPRICE,totalvalue);
            intent.putExtra(PaymentModeActivity.PLATFORM,PLATFORMWATER);
            startActivity(intent);
        }
